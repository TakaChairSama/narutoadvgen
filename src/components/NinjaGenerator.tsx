...
const NinjaGenerator: React.FC = () => {
  ...
  const [selectedNatures, setSelectedNatures] = useState<ChakraNature[]>([CHAKRA_NATURES[0]]);
  ...
  useEffect(() => {
    ...
    const file = clan.toString().toLowerCase().replace(/\s+/g, '');
    import(`../data/clans/${file}`)
      .then(module => {
        ...
        setClanFeatures(Array.isArray(f) ? f.map((x: any) => ({ name: x.name || 'Feature', description: x.description || '' })) : []);
  ...
  const handleGenerate = () => {
    ...
    const modifiers = Object.fromEntries(Object.entries(stats).map(([k, v]) => [k, calculateModifier(v)])) as Record<string, number>;
    ...
    const weapons = [{ name: 'Kunai', type: 'Thrown', damage: '1d4', properties: ['Thrown (30/60)', 'Light'], traits: [] }];
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        try {
          ...
          else alert('Imported file should be an array of characters');
        } catch {
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowGenerator(!showGenerator)} className="px-4 py-2 bg-orange-500 text-white rounded hover:bg-orange-600 transition flex items-center gap-2">
                <Plus className="w-5 h-5" />
                {showGenerator ? 'Hide' : 'Show'} Generator
              </button>
              {characters.length > 0 && (
                <>
                  <button onClick={exportCharacters} className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600 transition flex items-center gap-2">
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                ...
                <input type="text" value={customName} onChange={(e) => setCustomName(e.target.value)} className="w-full p-2 border rounded" placeholder="Leave blank for auto-name" />
              </div>
              ...
              <div>
                ...
                <select value={clan} onChange={(e) => setClan(e.target.value as NinjaClan)} className="w-full p-2 border rounded">
                  ...
                  {NINJA_CLANS.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              ...
              <div>
                ...
                <select value={specialty} onChange={(e) => setSpecialty(e.target.value as NinjaSpecialty)} className="w-full p-2 border rounded">
                  {NINJA_SPECIALTIES.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
              </div>
              ...
              <div className="md:col-span-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {CHAKRA_NATURES.map(nature => (
                    <button key={nature} onClick={() => handleNatureToggle(nature)} className={`px-4 py-2 rounded transition ${selectedNatures.includes(nature) ? 'bg-orange-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>  
                      {nature}
                    </button>
                  ))}
                </div>
            ...
            <button onClick={handleGenerate} className="mt-4 w-full py-3 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition font-semibold">Generate Character</button>
          </div>
        )}

        {/* Tabs and details (keeps layout & tracking from the suggestion, shows library data) */}
        {characters.length > 0 && (
          <div className="bg-white rounded-lg shadow-lg overflow-hidden">
            <div className="flex items-center gap-2 p-2 bg-gray-100 overflow-x-auto">
              <Users className="w-5 h-5 text-gray-600 flex-shrink-0" />
              {characters.map(char => (
                <button key={char.id} onClick={() => setActiveTab(char.id)} className={`flex items-center gap-2 px-4 py-2 rounded transition flex-shrink-0 ${ activeTab === char.id ? 'bg-orange-500 text-white' : 'bg-white text-gray-700 hover:bg-gray-200'}`}>  
                  {char.name}
                  <X className="w-4 h-4 hover:text-red-500" onClick={(e) => { e.stopPropagation(); handleDelete(char.id); }} />
                </button>
              ))}
            </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-red-50 p-4 rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => handleHPChange(activeCharacter.id, activeCharacter.hp - 1)} className="px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600">-</button>
                          <span>{activeCharacter.hp} / {activeCharacter.maxHp}</span>
                          <button onClick={() => handleHPChange(activeCharacter.id, activeCharacter.hp + 1)} className="px-2 py-1 bg-green-500 text-white rounded hover:bg-green-600">+</button>
                        </div>
                      </div>
                    ...
                    <div className="bg-blue-50 p-4 rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => handleChakraChange(activeCharacter.id, activeCharacter.chakra - 1)} className="px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">-</button>
                          <span>{activeCharacter.chakra} / {activeCharacter.maxChakra}</span>
                          <button onClick={() => handleChakraChange(activeCharacter.id, activeCharacter.chakra + 1)} className="px-2 py-1 bg-green-500 text-white rounded hover:bg-green-600">+</button>
                        </div>
                      </div>
                      <div className="w-full bg-gray-300 rounded-full h-4">
